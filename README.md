...
Altri caratteri tipografici che possono essere utili sono l'apostrofo ``’``, le virgolette basse doppie ``« »``, la linetta emme ``—`` e la barra verticale (pipe) spezzata ``¦``.
...
AutoHotKey consente di usare altri script molto utili per automatizzare molte operazioni (disponibili su https://www.autohotkey.com/boards/), occupa pochissima memoria nel sistema e se si volesse tornare alle impostazioni di default basta disattivare AutoHotKey o rimuovere lo script.
...
| `CTRL` + `SHIFT` + `` \|`` | `¦` | barra vertical spezzata |
